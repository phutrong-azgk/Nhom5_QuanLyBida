...
namespace Nhom5_QuanLyBida
{
    public partial class UserControl_QLKho : UserControl
    {
        ...
        private void btn_Xoa_Click(object sender, EventArgs e)
        {
            ...
            string selectedMaMon = txtMaMon.Text.Trim();

            var confirm = MessageBox.Show(
                $"Bạn có chắc chắn muốn xóa món '{txtTenMon.Text}'? Hành động này sẽ xóa toàn bộ lịch sử kho và chi tiết hóa đơn liên quan.",
            ...
            try
            {
                ...
                MessageBox.Show("Đã xóa món và các dữ liệu liên quan thành công!");
                LoadKho();

                // Clear các trường nhập liệu
                txtMaKho.Text = txtMaMon.Text = txtTenMon.Text = "";
                txtSLNhap.Text = txtSLXuat.Text = txt_DonGia.Text = "0";
            }
            catch (Exception ex)
            {
        ...
        private void btn_Luu_Click(object sender, EventArgs e)
        {
            ...
            catch (SqlException ex) // Thay đổi từ Exception thành SqlException
            {
                if (ex.Number == 547)
                {
                    if (ex.Message.Contains("CHK_SoLuongNhap"))
                        MessageBox.Show("Lỗi: Số lượng nhập vào không được nhỏ hơn 0!", "Vi phạm ràng buộc", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    else if (ex.Message.Contains("CHK_SoLuongXuat"))
                        MessageBox.Show("Lỗi: Số lượng xuất không được nhỏ hơn 0!", "Vi phạm ràng buộc", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    else
                        MessageBox.Show("Lỗi: Dữ liệu nhập vào vi phạm quy tắc kiểm tra của hệ thống!", "Thông báo lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
        else if (ex.Number == 2627 || ex.Number == 2601)
                {
                    MessageBox.Show($"Lỗi: Mã món '{txtMaMon.Text}' đã tồn tại trong danh sách kho. Vui lòng chọn món khác hoặc sử dụng chức năng Sửa!", "Trùng lặp dữ liệu", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    MessageBox.Show("Lỗi cơ sở dữ liệu: " + ex.Message, "Lỗi hệ thống", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
