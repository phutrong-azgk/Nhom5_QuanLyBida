...
namespace Nhom5_QuanLyBida
{
    public partial class UserControl_KhuyenMai : UserControl
    {
        ...
        {
            if (currentlySelectedPromotion == null || string.IsNullOrEmpty(selectedPromotionId))
            {
            ...
            if (result == DialogResult.Yes)
            {
                try
                {
                    ...
                    {
                        conn.Open();
                        using (SqlCommand cmd = new SqlCommand("sp_XoaKhuyenMai", conn))
                        {
                ...
                {
                    MessageBox.Show($"Lỗi khi thực thi xóa: {ex.Message}");
                }
            }
        }
    }
